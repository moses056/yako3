...
export default function CreateContract({ location }) {
    ...
    const startContract = async () => {
        ...
        setDone(true)
        const q = query(collection(db, "talent", talentID, "jobProposal"), where("jobId", "==", jobID));
        const querySnapshot = await getDocs(q);
        if (querySnapshot.docs[0]) {
            await updateDoc(doc(db, "talent", talentID, "jobProposal", querySnapshot.docs[0].id), { 
                status: "offer", 
                budget: contract.jobBudget || job.jobBudget 
            })
        }

        await addDoc(collection(db, "talent", talentID, "notification"), {

...
export default function SubmitProposal() {
  ...
  const handlewithdrawProposal = async () => {
    try {
      const q = query(collection(db, "job", id, "proposals"), where("talentId", "==", auth.currentUser.uid));
      const querySnapshot = await getDocs(q);
      querySnapshot.docs.map((e) => {
        proposal = e.id;
        setProposal(proposal);
        deleteDoc(doc(db, "job", id, "proposals", proposal));
        console.log(proposal);
      });

      const q2 = query(collection(db, "talent", auth.currentUser.uid, "jobProposal"), where("jobId", "==", id));
      const querySnapshot2 = await getDocs(q2);
      querySnapshot2.docs.map((e) => {
        talent = e.id;
        setTalent(talent);
        deleteDoc(doc(db, "talent", auth.currentUser.uid, "jobProposal", talent));
        console.log(talent);
      });
    } catch (err) {
      console.log(err);

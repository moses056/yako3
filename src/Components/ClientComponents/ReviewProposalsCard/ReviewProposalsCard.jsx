...
export default function ReviewProposalsCard() {
  ...
  useEffect(() => {
    ...
    const proposalsCollection = collection(jobRef, "proposals");
    
    getDocs(proposalsCollection).then(async (querySnapshot) => {
      const proposalsData = [];
      
      for (const proposalDoc of querySnapshot.docs) {
        const proposalData = proposalDoc.data();
        proposalsData.push(proposalData);
        
        if (proposalData.talentId) {
          const talentRef = doc(db, "talent", proposalData.talentId);
          const talentDoc = await getDoc(talentRef);
          
          if (talentDoc.exists()) {
            arr.push(talentDoc.data());
          }
        }
      }
